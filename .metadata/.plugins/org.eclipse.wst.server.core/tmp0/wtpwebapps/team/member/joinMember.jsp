...
<jsp:include page="/header.jsp" />
  <section class="section">
	...
	<form name="frm1" id="frm1" action="${path1 }/JoinMemberCtrl.do" method="post" onsubmit="return joinCheck(this)">
		<table class="table">
			<tbody>
				<tr>
					<th>아이디</th>
				...
				<tr>
					<th>연락처</th>
					<td><input type="tel" name="tel" id="tel" maxlength="11" placeholder="숫자만 입력 " class="input" required></td>
				</tr>
				<tr>
					<th>이메일</th>
					<td>
						<div class="form-row">
							<input type="text" name="email" id="email" placeholder="아이디 @intranet.com" class="input" required />
						 	<input type="button" class="button" value="이메일 중복 확인" onclick="emailCheck()">
							<input type="hidden" name="emailCk" id="emailCk" value="no">
						</div>
						<div>
							<c:if test="${empty qemail }">
							<p id="msg2" style="padding-left:0.5rem">아직 이메일 중복 체크를 하지 않으셨습니다.</p>
							</c:if>
							<c:if test="${not empty qemail }">
							<p id="msg2" style="padding-left:0.5rem">이메일 중복 체크 후 수정하였습니다.</p>
							</c:if>
						</div> 
					</td>
				</tr>
				<tr>
					<th>주소</th>
					<td><input type="text" name="addr" id="addr" placeholder="주소 입력" class="input" required /><br>
					</td>
				</tr>
			</tbody>
		</table>
		<div class="btn-group">
			<input type="submit" name="submit-btn" class="button" value="가입">
			<a href="<%=request.getContextPath() %>/GetMemberListCtrl.do">회원 목록</a>
			<a href="<%=request.getContextPath() %>./login.jsp" class="button">로그인</a>
		</div>
	</form>	
	...
	$(document).ready(function(){
		$("#email").keyup(function(){
			...
			if($(this).val()!=""){
				...
				$("#msg2").html("아직 이메일 중복 체크를 하지 않으셨습니다.");
			}
		});
	});
	</script>
		...
		$.ajax({
			...
			success:function(result){
				...
				if(emailChk==false){	
					...
					$("#emailCk").val("yes");
					$("#msg2").html("<strong style='color:blue'>사용가능한 이메일 입니다.</strong>");
				} else if(emailCk==""){
